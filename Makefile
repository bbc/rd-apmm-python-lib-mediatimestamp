...
TOXENV?=py36
TOXACTIVATE=$(TOXDIR)/$(TOXENV)/bin/activate
...
$(TOXDIR)/%/bin/activate: tox.ini
	tox -e $* --recreate --workdir $(TOXDIR)
...
$(topbuilddir)/docs/$(MODNAME).html: $(topbuilddir)/docs/$(MODNAME) $(TOXACTIVATE)
	. $(TOXACTIVATE) && cd $(topbuilddir)/docs/ && pydoc -w ./
